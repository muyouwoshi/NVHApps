#include "com_calculate_Arith_FFT.h"
#include "fft.h"
#include "rpm.h"
...
jint native;
using namespace std;
JNIEXPORT jint JNICALL Java_com_calculate_Arith_1FFT_init(JNIEnv * env,
		jobject obj) {
	FFTParam param;
	if (!ifChangeParam) {
		InitFFtParam(param);
...
JNIEXPORT void JNICALL Java_com_calculate_Arith_1FFT_finalizer(JNIEnv * env,
