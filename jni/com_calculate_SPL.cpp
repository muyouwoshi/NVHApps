#include "com_calculate_Arith_SPL.h"
#include "Level.h"
#include <MeanMax.h>
#include "Common.h"
using namespace std;
bool ifChangeParam;
jint native;
JNIEXPORT jint JNICALL Java_com_calculate_Arith_1SPL_init(JNIEnv * env,
		jobject obj) {
	LevelParam param;
	if (!ifChangeParam) {
...
JNIEXPORT void JNICALL Java_com_calculate_Arith_1SPL_finalizer(JNIEnv * env,
		jobject obj, jint classz) {
	CLevel *leverAve = (CLevel*) classz;
	delete leverAve;
}

JNIEXPORT jint JNICALL Java_com_calculate_Arith_1SPL_setWeight(JNIEnv * env,
		jobject obj, jint classz, jint weight) {
	CLevel *leverAve = (CLevel*) classz;
...
JNIEXPORT jint JNICALL Java_com_calculate_Arith_1SPL_setAva(JNIEnv * env,
		jobject obj, jint classz, jint avg) {
	CLevel *leverAve = (CLevel*) classz;
	LevelParam param = leverAve->GetM_param();
	switch (avg) {
	case 0:
